...
function filterHistory() {
    ...
    const filtered = historyData.filter(item => {
        ...
        const displayStatus = (item.displayStatus || record.displayStatus || record.status || "").toLowerCase().trim();
        ...
        if (status !== "all") {
            if (status === "rented" && displayStatus !== "đã thuê") return false;
            if (status === "active" && !(displayStatus === "đang thuê" || displayStatus.includes("xác nhận"))) return false;
            if (status === "returned" && displayStatus !== "đã trả xe") return false;
        }
...
function updateAnalyticsFromHistory(list) {
    ...
    const durationsInDays = list
        .map(item => {
            const start = parseDate(item.record?.startTime);
            const end = parseDate(item.record?.endTime);
            return start && end ? (end - start) / (1000 * 60 * 60 * 24) : null;
        })
        .filter(v => v !== null);

    const avgDuration = durationsInDays.length
        ? durationsInDays.reduce((a, b) => a + b, 0) / durationsInDays.length
        : 0;
    ...
    document.getElementById("avg-duration").innerHTML = `${avgDuration.toFixed(1)} <small>ngày</small>`;
}
...
async function loadStats() {
    try {
        ...
        const avgDays = typeof stats.averageDurationDays === "number"
            ? stats.averageDurationDays
            : (stats.averageDurationMinutes || 0) / (60 * 24);
        document.getElementById("avg-duration").innerHTML = `${avgDays.toFixed(1)} <small>ngày</small>`;
    } catch (err) {
